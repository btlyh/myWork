...
public class CardManager
{
	...
	/** 卡牌吞噬 */
	public void engulfCard(Player player,int cardId,
		ArrayList<Integer> list,ByteBuffer data)
	{
		Card card=player.getCardBag().getById(cardId);
		String error=checkCard(card);
		if(error!=null)
		...
		if(card.isRealm())
		{
			...
			if(isbeyong)//境界成功突破
			{
				...
				card.setBeyongAddAct(card.getBeyongAddAct()+beyongAct);//每突破一次提升基础攻击力的4% * 奖励次数
				card.setBeyongAddHp(card.getBeyongAddHp()+beyongHp);//每突破一次提升基础气血的4% * 奖励次数
				card.setBeyongAddDodge(card.getBeyongAddDodge()+beyongDodge);//每突破一次提升基础气血的1% * 奖励次数
		...
		else
		{
			...
			if(levelNum==0)//未升级 仅仅提升经验
			{
			  ...
			  if(GameCFG.getLevelExp().length+1==card.getLevel())//达到满级	  
			  {
				  data.writeInt(0);
			  }else
			  {
			...
			{
				card.setBeyongAddAct(card.getBeyongAddAct()+act);//每突破一次提升基础攻击力的4% * 奖励次数
				card.setBeyongAddHp(card.getBeyongAddHp()+hp);//每突破一次提升基础气血的4% * 奖励次数
				card.setBeyongAddDodge(card.getBeyongAddDodge()+dodge);//每突破一次提升基础气血的1% * 奖励次数
	 ...
	 * @param list 吞噬卡牌id链表
	 */
	private void levelUp(Player player,int cardId,
		...
		ArrayList<Integer> cardList)
	{
		...
		for(int i=0;i<cardList.size();i++)
		{
			 ...
			 if (!checkcard)
			 {
			  ...
			  for (int j = 0; j < limit.length; j++) 
			  { 
				  boolean ishave = false;//玩家是否已经吞噬了这张卡牌了
				 if(limit[j]==card_.getSid())
				 {
					 ...
					 if(!ishave)
					 {
						 card.getEngulfCards().add(card_.getSid());//将该条件卡牌加入到已经吞噬的卡牌中去
					 }else
					 {
						 return Lang.F1212;
					 }	 
