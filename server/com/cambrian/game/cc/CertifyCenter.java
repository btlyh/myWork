	...
	TimerListener,CloseAble
{
	...
	/** 无效的用户名或密码 */
	public static final String err3=CertifyCenter.class.getName()
		+" certify, invalid name or password";
	/** 无效的sid */
		...
		Session session)
	{
		...
		try
		{
			...
			if(b)
			{
				...
				String str = (String)access.valid(id, pw, serverId, address);// uid:serverId
				if(str == null)
					throw new DataAccessException(
						DataAccessException.SERVER_CMSG_ERROR,err1);
				...
				s = new Session(id);
				s.setSource(sid);
				s.setReference(strs[0]);// uid
				s.setAttribute("serverId", strs[1]);// serverId
				...
				if(log.isInfoEnabled())
					log.info("certify ok, id="+id+", sid="+sid+", uid="
						+strs[0]+", gameserver="+strs[1]+", "+session);
			}
			else
			{
