	...
	CloseAble
{
	...
	/** 获得数据中心访问器 */
	public DSDCAccess getDSDCAccess()
	{
		return dcAccess;
	}
	/** 设置数据中心访问器 */
	public void setDSDCAccess(DSDCAccess access)
	{
		dcAccess=access;
	}
	/** 获得认证中心访问器 */
	...
	public void login(String sid,boolean renew,NioTcpConnect connect)
	{
		...
		if(!dcAccess.canAccess())
			throw new DataAccessException(
				DataAccessException.SERVER_INTERNAL_ERROR,err3);
	...
	{
		ByteBuffer bb=dcAccess.getRandomName();
		return bb;
	}
	/** 加载 */
		...
		String userName)
	{
		...
		if(!dcAccess.canAccess())
			throw new DataAccessException(
				DataAccessException.SERVER_INTERNAL_ERROR,err11);
		synchronized(saveList)
		{
			if(saveList.contain(uid))
				throw new DataAccessException(
		...
		ByteBuffer bb=dcAccess.load(uid,nickName,userName);
	...
	public void exit(Session session,ByteBuffer data)
	{
		...
		if(!(dcAccess.canAccess()))
			throw new DataAccessException(
				DataAccessException.SERVER_INTERNAL_ERROR,err21);
		synchronized(saveList)
		{
	...
	void collate(long time)
	{
		...
		boolean dcAccess=this.dcAccess.canAccess();
		...
		ByteBuffer data)
	{
		...
		try
		{
			...
			dcAccess.save(uid,exit,save?data:null);
			s.setState(CRC_STATE,crc);
			if(log.isInfoEnabled())
