...
public class ArmyCampManager
{
	...
	public void useAwakeSoup(Player player, String userName)
	{
		...
		tarArmyCamp.addHistoryLog("【(" + player.getNickname()
	...
	public Map<String, Object> enterArmyCamp(Player player, String userName)
	{
		...
		for (int i = 0; i < publicList.size(); i++)
		{
			...
			if (TimeKit.nowTimeMills() - card.getLastDrinkTime() >= ArmyCamp.DRINK_CD)
			{
				...
				Session noticeSession = ds.getSession(seatCard.getOwnerName());
			...
			boolean b)
	{
		...
		for (int i = 0; i < cards.size(); i++)
		{
			...
			if (card.getDrinkStatus() == Card.DRUNK) // 醉酒卡牌处理
			{
				...
				{
					if (card.getArmyName().equals(player.getNickname()))
					{
						removeMyCard(player, cards.get(i));
						--i;
						continue;
					} else
					{
				...
				{
					payForAwake(player, card.getArmyName(), cards.get(i),
							countUseGold(card), b);
				}
			}
		}
	...
	/** 计算所需金币 */
	private int countUseGold(Card card)
	{
		int cdSeconds = TimeKit.timeSecond(ArmyCamp.DRINK_CD
				- (TimeKit.nowTimeMills() - card.getLastDrinkTime()));
